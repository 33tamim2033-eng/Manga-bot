...
SECOND_OWNER_ID = 7461461159                # 👈 آيدي المالك الثاني الذي طلبت إضافته
# مجموعة المالكين (تسمح بامتدادية مستقبلية)
...
(
    ...
    DELETE_SELECT       # انتظار اختيار الحذ��
) = range(10)
...
def init_db():
    ...
    # تسجيل المالكين كأدمن وحصانته
    for owner in OWNERS:
        c.execute("INSERT OR IGNORE INTO users (user_id, is_admin, points) VALUES (?, 1, 999999)", (owner,))
        c.execute("UPDATE users SET is_admin = 1, is_banned = 0 WHERE user_id = ?", (owner,))
...
async def check_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # استثناء الأدمن والمالكين من الفحص
    status = check_user_status(user_id)
    if user_id in OWNERS or status['is_admin']:
        return True
...
async def ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if target_id in OWNERS:
            await update.message.reply_text("⛔ لا يمكنك حظر أحد المالكين.")
            return
...
async def get_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("4️⃣ أرسل <b>التقييم</b> (مثلًا: 8.5/10):", parse_mode=ParseMode.HTML)
    return RATING
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
       "📚 <b>مرحبًا بك في بوت مملكة المانجا! (V5.0)</b>\n"
       "دليلك الشامل وأضخم نظام لجميع ألاعمال M3C 🔥\n"
...
def main():
    ...
    # تشغيل خادم الويب للحفاظ على عمل البوت 24/7 (خاص بـ Render)
    keep_alive()
