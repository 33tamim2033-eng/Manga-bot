...
# يحاول البوت أولاً سحب التوكن من إعدادات ريندر، وإذا لم يجده يستخدم التوكن المكتوب
TOKEN = os.getenv("TOKEN", "8514548815:AAFEdazZosjj5oKrYhZ09nnXtDCbd6ZNWBc")           # 👈 توكن البوت (fallback)
...
# ==============================================================================
#                       Flask "I am alive" (لـ Render)
# ==============================================================================
# خادم ويب بسيط ليعتقد Render أن التطبيق موقع ويب، يمنع توقف الخدمة.
...
def run_web():
    # ريندر يطلب من البوت العمل على بورت معين
...
def extract_chapter_number(text):
    """
    الدالة الذكية الشاملة:
    - تعالج 1-20 كفصل مدمج.
    - تعالج 165-5 كفصل عشري (165.5).
    - تتجاهل الأرقام الزائدة في نهاية اسم الملف.
    """
    if not text:
        return "0"
    ...
    # 1. البحث عن الصيغة العشرية الصريحة أولاً (ch 165.5)
    # لضمان عدم تكسيرها
    decimal_match = re.search(r'(?:ch|chapter|ep|vol)\D*?(\d+\.\d+)', text_clean)
    if decimal_match:
        return decimal_match.group(1)

    # 2. البحث عن صيغة الشرطة (X-Y) والتحقق بالمنطق
    # يكتشف: ch 1-20 وأيضاً ch 165-5
    hyphen_match = re.search(r'(?:ch|chapter|ep|vol)\D*?(\d+)\s*-\s*(\d+)', text_clean)
    if hyphen_match:
        num1 = int(hyphen_match.group(1))
        num2 = int(hyphen_match.group(2))
        
        # اللحظة الحاسمة: المقارنة
        if num1 < num2:
            return f"{num1}-{num2}"  # مثال 1-20 (مدمج)
        else:
            return f"{num1}.{num2}"  # مثال 165-5 (يصبح 165.5)

    # 3. الفصول الفردية العادية
    single_match = re.search(r'(?:ch|chapter|ep|vol|#)\D*?(\d+)', text_clean)
    if single_match:
        return single_match.group(1)

    # 4. محاولة أخيرة للنطاقات بدون كلمة ch (للملفات التي هي أرقام فقط)
    plain_range = re.search(r'(\d+)\s*-\s*(\d+)', text_clean)
    if plain_range:
        n1 = int(plain_range.group(1))
        n2 = int(plain_range.group(2))
        if n1 < n2:
            return f"{n1}-{n2}"
        else:
            return f"{n1}.{n2}"
...
# ==============================================================================
#                           أ��امر لوحة التحكم (Admin Dashboard)
# ==============================================================================
...
async def archive_forwarded_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    manga_id = context.user_data.get('index_manga_id')
    is_merged = context.user_data.get('is_merged', 0)
...
def main():
    ...
    application.add_handler(conv_index)
    ...
    application.add_handler(CommandHandler("promote", promote_admin))
    application.add_handler(CommandHandler("demote", demote_admin))
    application.add_handler(CommandHandler("ban", ban_user))
    application.add_handler(CommandHandler("unban", unban_user))
    application.add_handler(CommandHandler("request", request_manga))
    application.add_handler(CommandHandler("search", smart_search))
    application.add_handler(CommandHandler("cancel", cancel)) # أمر هام للخروج من أي تعليق
    
    # التعامل مع الرسائل النصية (للبحث السريع)
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, smart_search))
    ...
    # هذا السطر سيجعل الموقع يعمل في الخلفية بينما البوت يشتغل
